...
# The certificate will be issued for the Common Name (CN) of `webhook-server.microapplication-webhook.svc`, which is the
...
openssl req -new -key webhook-server-tls.key -subj "/CN=webhook-server.microapplication-webhook.svc" \
